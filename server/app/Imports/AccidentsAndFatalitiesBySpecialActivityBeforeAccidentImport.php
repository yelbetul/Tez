...
class AccidentsAndFatalitiesBySpecialActivityBeforeAccidentImport implements ToCollection, WithHeadingRow
{
    public $failures = [];
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            $validator = Validator::make($row->toArray(), [
                'year' => 'required',
                'group_id' => 'required|integer|exists:special_activities_before_accidents,id',
        ...
        if (empty($this->failures)) {
            AccidentsAndFatalitiesBySpecialActivityBeforeAccident::insert($dataToInsert);
        }
    }
