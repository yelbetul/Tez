...
class WorkAccidentByWorkstationImport implements ToCollection, WithHeadingRow
{
    public $failures = [];
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            $validator = Validator::make($row->toArray(), [
                'year' => 'required',
                'group_id' => 'required|integer|exists:workstation_types,id',
        ...
        if (empty($this->failures)) {
            WorkAccidentByWorkstation::insert($dataToInsert);
        }
    }
