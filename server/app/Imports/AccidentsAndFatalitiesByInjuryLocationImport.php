...
class AccidentsAndFatalitiesByInjuryLocationImport implements ToCollection, WithHeadingRow
{
    public $failures = [];
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            $validator = Validator::make($row->toArray(), [
                'year' => 'required',
                'group_id' => 'required|integer|exists:injury_locations,id',
        ...
        if (empty($this->failures)) {
            AccidentsAndFatalitiesByInjuryLocation::insert($dataToInsert);
        }
    }
