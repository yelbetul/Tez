...
use App\Models\AccidentsAndFatalitiesByEmployee;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;
use App\Imports\AccidentsAndFatalitiesByEmployeeImport;
use Maatwebsite\Excel\Facades\Excel;
use Maatwebsite\Excel\Validators\ValidationException;
use App\Helpers\AnalysisHelperAccidentsByEmployees;
use OpenAI\Laravel\Facades\OpenAI;
use Illuminate\Support\Facades\DB;


class AccidentsAndFatalitiesByEmployeeController extends Controller
{
    /**
     * Validasyon
     */
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::with('employeeGroup')->get();
        return response()->json($data);
    }
    public function indexUser(Request $request)
    {
        $query = DB::table('accidents_and_fatalities_by_employees')
            ->join('employee_groups', 'accidents_and_fatalities_by_employees.group_id', '=', 'employee_groups.id')
            ->select(
                'accidents_and_fatalities_by_employees.year',
        ...
        if ($request->has('year') && $request->year !== 'all') {
            $query->where('accidents_and_fatalities_by_employees.year', $request->year);
        }
        ...
        if ($request->has('gender') && $request->gender !== 'all') {
            $query->where('accidents_and_fatalities_by_employees.gender', $request->gender);
        }
        ...
        $prompt = AnalysisHelperAccidentsByEmployees::buildAIPrompt($request, $summary);
        $analysis = AnalysisHelperAccidentsByEmployees::getAICommentary($prompt);
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::where('year', $year)->with('employeeGroup')->get();
        return response()->json($data);
    }
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::where('group_id', $groupId)->with('employeeGroup')->get();
        return response()->json($data);
    }
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::where('gender', $gender)->with('employeeGroup')->get();
        return response()->json($data);
    }
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::whereHas('employeeGroup', function ($query) use ($injuryCode) {
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::whereHas('employeeGroup', function ($query) use ($groupCode) {
    ...
    {
        $data = AccidentsAndFatalitiesByEmployee::whereHas('employeeGroup', function ($query) use ($subGroupCode) {
    ...
    public function store(Request $request)
    {
        ...
        $record = new AccidentsAndFatalitiesByEmployee();
    ...
    public function update(Request $request, $id)
    {
        ...
        $record = AccidentsAndFatalitiesByEmployee::find($id);
        if (!$record) {
...
public function import(Request $request)
    {
        ...
        try {
            $import = new AccidentsAndFatalitiesByEmployeeImport;
