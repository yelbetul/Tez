...
use App\Helpers\AnalysisHelperAccidentsByMaterials;
...
class AccidentsAndFatalitiesByMaterialController extends Controller
{
    ...
    {
        $query = DB::table('accidents_and_fatalities_by_materials')
            ->join('materials', 'accidents_and_fatalities_by_materials.group_id', '=', 'materials.id')
            ->select(
                'accidents_and_fatalities_by_materials.year',
                'materials.group_code',
                'materials.group_name',
                'materials.sub_group_code',
                'materials.sub_group_name',
        ...
        if ($request->has('year') && $request->year !== 'all') {
            $query->where('accidents_and_fatalities_by_materials.year', $request->year);
        }

        if ($request->has('group_code') && $request->group_code !== 'all') {
            $query->where('materials.group_code', $request->group_code);
        }

        if ($request->has('sub_group_code') && $request->sub_group_code !== 'all') {
            $query->where('materials.sub_group_code', $request->sub_group_code);
        }

        if ($request->has('material_code') && $request->material_code !== 'all') {
            $query->where('materials.material_code', $request->material_code);
        }

        if ($request->has('gender') && $request->gender !== 'all') {
            $query->where('accidents_and_fatalities_by_materials.gender', $request->gender);
        }
        ...
        $prompt = AnalysisHelperAccidentsByMaterials::buildAIPrompt($request, $summary);
        $analysis = AnalysisHelperAccidentsByMaterials::getAICommentary($prompt);
