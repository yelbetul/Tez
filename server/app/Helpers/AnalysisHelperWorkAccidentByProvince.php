...
class AnalysisHelperWorkAccidentByProvince
{
    public static function buildAIPrompt($request, $summary)
    {
        ...
        $prompt = "Aşağıdaki şehir bazlı iş kazası verilerini ISO 45001 standartlarına uygun teknik raporda analiz et:\n\n";
        ...
        $prompt .= "   - Bölgesel Vaka Sıklık Oranı (RFR) hesaplaması\n";
        $prompt .= "   - Coğrafi risk haritalandırması (5x5 matris)\n\n";
        
        $prompt .= "2. Kök Neden Analizi (ISO 45002:2016):\n";
        $prompt .= "   - Bölgesel faktörler için Fishbone diyagramı\n";
        $prompt .= "   - İl bazlı endüstriyel dağılım analizi\n\n";
        
        $prompt .= "3. Demografik Analiz (ILO C161 Uyumlu):\n";
        $prompt .= "   - Şehirler arası cinsiyet dağılımı karşılaştırması\n";
        $prompt .= "   - Bölgesel iş gücü demografisi\n\n";
        
        $prompt .= "4. İyileştirme Planı (Hierarchy of Controls):\n";
        $prompt .= "   - Bölgesel güvenlik önlemleri (EN ISO 12100)\n";
        $prompt .= "   - Yerel yönetim işbirliği stratejileri (PDCA döngüsüne uygun)\n\n";
        
        $prompt .= "5. Projeksiyon (ISO 45001 Madde 6.1):\n";
        $prompt .= "   - Bölgesel 3 yıllık trend analizi (ARIMA model önerisi)\n\n";
        ...
        $prompt .= "- Tüm hesaplamalarda formül belirt (Örn: RFR=(Bölgesel Vaka Sayısı/Çalışılan Saat)x1.000.000)";
    ...
    private static function generateQuantitativeDataSection(array $summary): string
    {
        ...
        return "KANTİTATİF VERİLER (ISO 39001 Formatında):\n" .
               "- Toplam Vaka: " . ($summary['total_accidents'] ?? 0) . " (n)\n" .
               "- İş Göremezlik Vakaları: " . ($summary['total_unfit'] ?? 0) . " (%".$unfitPercentage.")\n" .
               "- Meslek Hastalıkları: " . ($summary['total_diseases'] ?? 0) . " (ICD-10 koduyla)\n" .
               "- Cinsiyet Dağılımı:\n" .
                ...
                'messages' => [
                    [
                        ...
                        'content' => "Sen bir ISO 45001 Sertifikalı İş Güvenliği Uzmanısın. "
                            ...
                            . "1. Bölgesel/şehir bazlı analiz yap\n"
