...
class AnalysisHelperAccidentsByDeviations
{
    public static function buildAIPrompt($request, $summary)
    {
        $year = self::formatRequestInput($request->input('year'), 'Tüm yıllar');
        $groupCode = self::formatRequestInput($request->input('group_code'), 'Tüm sapma grupları');
        $subGroupCode = self::formatRequestInput($request->input('sub_group_code'), 'Tüm alt gruplar');
        $deviationCode = self::formatRequestInput($request->input('deviation_code'), 'Tüm sapma türleri');

        $prompt = "Aşağıdaki sapma türlerine göre iş kazası ve ölüm verilerini ISO 45001 standartlarına uygun teknik raporda analiz et:\n\n";
        $prompt .= "VERİ PARAMETRELERİ (ISO 45003:2021 Formatında):\n";
        $prompt .= "- Analiz Periyodu: " . self::formatISODateRange($year) . "\n";
        $prompt .= "- Sapma Grubu: " . self::formatActivityCode($groupCode) . "\n";
        $prompt .= "- Alt Grup: " . self::formatActivityCode($subGroupCode) . "\n";
        $prompt .= "- Sapma Türü: " . self::formatActivityCode($deviationCode) . "\n\n";
        ...
        $prompt .= "   - Sapma türlerine göre kaza sıklık oranı\n";
        $prompt .= "   - Ölümlü kazalar için kritik risk haritalandırması (5x5 matris)\n";
        $prompt .= "   - İş göremezlik sürelerine göre sapma risk sıralaması\n\n";

        $prompt .= "2. Kök Neden Analizi (ISO 45002:2016):\n";
        $prompt .= "   - Sapma gruplarına özgü kaza nedenleri (Fishbone diyagramı)\n";
        $prompt .= "   - Ölümlü kazalar için ILO sınıflandırmasıyla analiz\n\n";

        $prompt .= "3. Demografik Analiz (ILO C161 Uyumlu):\n";
        $prompt .= "   - Sapma türlerine göre cinsiyet dağılımı\n";
        $prompt .= "   - Ölümlü kazalarda sapma risk faktörleri\n\n";

        $prompt .= "4. İyileştirme Planı (Hierarchy of Controls):\n";
        $prompt .= "   - Sapma türlerine özgü mühendislik kontrolleri (EN ISO 12100)\n";
        $prompt .= "   - Kritik sapma türleri için acil önlem paketleri\n\n";

        $prompt .= "5. Risk Projeksiyonu:\n";
        $prompt .= "   - Sapma gruplarına göre 3 yıllık kaza trend analizi\n\n";

        $prompt .= "RAPOR FORMAT KURALLARI:\n";
        $prompt .= "- Tamamen ISO 45001 terminolojisi kullan\n";
        $prompt .= "- Sapma kodlarını ILO standardına göre belirt\n";
    ...
    public static function getAICommentary($prompt)
    {
        try {
                ...
                'messages' => [
                    [
                        ...
                        'content' => "Sen bir ISO 45001 Sertifikalı İş Güvenliği Uzmanısın. "
                            ...
                            . "1. Sapma türlerine göre iş kazaları ve ölüm verilerine odaklan\n"
                            . "2. ILO sapma sınıflandırmasını kullan\n"
            ...
            return "ISO 45001 uyumlu sapma analizi geçici olarak mevcut değil. Lütfen teknik ekibe bildiriniz.";
        }
    }
