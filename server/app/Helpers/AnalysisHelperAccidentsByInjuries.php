...
class AnalysisHelperAccidentsByInjuries
{
    public static function buildAIPrompt($request, $summary)
    {
        $year = self::formatRequestInput($request->input('year'), 'Tüm yıllar');
        $groupCode = self::formatRequestInput($request->input('group_code'), 'Tüm yaralanma grupları');
        $subGroupCode = self::formatRequestInput($request->input('sub_group_code'), 'Tüm alt gruplar');
        $injuryCode = self::formatRequestInput($request->input('injury_code'), 'Tüm yaralanma türleri');

        $prompt = "Aşağıdaki yaralanma türlerine göre iş kazası ve ölüm verilerini ISO 45001 standartlarına uygun teknik raporda analiz et:\n\n";
        $prompt .= "VERİ PARAMETRELERİ (ISO 45003:2021 Formatında):\n";
        $prompt .= "- Analiz Periyodu: " . self::formatISODateRange($year) . "\n";
        $prompt .= "- Yaralanma Grubu: " . self::formatInjuryCode($groupCode) . "\n";
        $prompt .= "- Alt Grup: " . self::formatInjuryCode($subGroupCode) . "\n";
        $prompt .= "- Yaralanma Türü: " . self::formatInjuryCode($injuryCode) . "\n\n";
        ...
        $prompt .= "   - Yaralanma türlerine göre kaza sıklık oranı\n";
        $prompt .= "   - Ölümlü kazalar için kritik risk haritalandırması (5x5 matris)\n";
        $prompt .= "   - İş göremezlik sürelerine göre yaralanma risk sıralaması\n\n";

        $prompt .= "2. Kök Neden Analizi (ISO 45002:2016):\n";
        $prompt .= "   - Yaralanma gruplarına özgü kaza nedenleri (Fishbone diyagramı)\n";
        $prompt .= "   - Ölümlü kazalar için ICD-10 sınıflandırmasıyla analiz\n\n";

        $prompt .= "3. Demografik Analiz (ILO C161 Uyumlu):\n";
        $prompt .= "   - Yaralanma türlerine göre cinsiyet dağılımı\n";
        $prompt .= "   - Ölümlü kazalarda yaralanma risk faktörleri\n\n";

        $prompt .= "4. İyileştirme Planı (Hierarchy of Controls):\n";
        $prompt .= "   - Yaralanma türlerine özgü mühendislik kontrolleri (EN ISO 12100)\n";
        $prompt .= "   - Kritik yaralanma türleri için acil önlem paketleri\n\n";

        $prompt .= "5. Risk Projeksiyonu:\n";
        $prompt .= "   - Yaralanma gruplarına göre 3 yıllık kaza trend analizi\n\n";

        $prompt .= "RAPOR FORMAT KURALLARI:\n";
        $prompt .= "- Tamamen ISO 45001 terminolojisi kullan\n";
        $prompt .= "- Yaralanma kodlarını ICD-10 standardına göre belirt\n";
    ...
    private static function formatInjuryCode($input): string
    {
        if (str_contains($input, ',')) {
    ...
    public static function getAICommentary($prompt)
    {
        try {
                ...
                'messages' => [
                    [
                        ...
                        'content' => "Sen bir ISO 45001 Sertifikalı İş Güvenliği Uzmanısın. "
                            ...
                            . "1. Yaralanma türlerine göre iş kazaları ve ölüm verilerine odaklan\n"
            ...
            return "ISO 45001 uyumlu yaralanma analizi geçici olarak mevcut değil. Lütfen teknik ekibe bildiriniz.";
        }
    }
