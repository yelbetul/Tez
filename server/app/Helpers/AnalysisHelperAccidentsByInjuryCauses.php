...
class AnalysisHelperAccidentsByInjuryCauses
{
    public static function buildAIPrompt($request, $summary)
    {
        $year = self::formatRequestInput($request->input('year'), 'Tüm yıllar');
        $groupCode = self::formatRequestInput($request->input('group_code'), 'Tüm yaralanma neden grupları');
        $subGroupCode = self::formatRequestInput($request->input('sub_group_code'), 'Tüm alt gruplar');
        $injuryCauseCode = self::formatRequestInput($request->input('injury_cause_code'), 'Tüm yaralanma nedenleri');

        $prompt = "Aşağıdaki yaralanma nedenlerine göre iş kazası ve ölüm verilerini ISO 45001 standartlarına uygun teknik raporda analiz et:\n\n";
        $prompt .= "VERİ PARAMETRELERİ (ISO 45003:2021 Formatında):\n";
        $prompt .= "- Analiz Periyodu: " . self::formatISODateRange($year) . "\n";
        $prompt .= "- Yaralanma Nedeni Grubu: " . self::formatInjuryCauseCode($groupCode) . "\n";
        $prompt .= "- Alt Grup: " . self::formatInjuryCauseCode($subGroupCode) . "\n";
        $prompt .= "- Yaralanma Nedeni: " . self::formatInjuryCauseCode($injuryCauseCode) . "\n\n";
        ...
        $prompt .= "   - Yaralanma nedenlerine göre kaza sıklık oranı\n";
        $prompt .= "   - Ölümlü kazalar için kritik risk haritalandırması (5x5 matris)\n";
        $prompt .= "   - İş göremezlik sürelerine göre yaralanma nedeni risk sıralaması\n\n";

        $prompt .= "2. Kök Neden Analizi (ISO 45002:2016):\n";
        $prompt .= "   - Yaralanma neden gruplarına özgü kaza senaryoları (Fishbone diyagramı)\n";
        $prompt .= "   - Ölümlü kazalar için ILO kaza sınıflandırmasıyla analiz\n\n";

        $prompt .= "3. Demografik Analiz (ILO C161 Uyumlu):\n";
        $prompt .= "   - Yaralanma nedenlerine göre cinsiyet dağılımı\n";
        $prompt .= "   - Ölümlü kazalarda yaralanma nedeni faktörleri\n\n";

        $prompt .= "4. İyileştirme Planı (Hierarchy of Controls):\n";
        $prompt .= "   - Yaralanma nedenlerine özgü önleyici tedbirler (EN ISO 12100)\n";
        $prompt .= "   - Kritik yaralanma nedenleri için acil önlem paketleri\n\n";

        $prompt .= "5. Risk Projeksiyonu:\n";
        $prompt .= "   - Yaralanma neden gruplarına göre 3 yıllık kaza trend analizi\n\n";

        $prompt .= "RAPOR FORMAT KURALLARI:\n";
        $prompt .= "- Tamamen ISO 45001 terminolojisi kullan\n";
        $prompt .= "- Kaza nedenlerini ILO sınıflandırmasına göre belirt\n";
    ...
    private static function formatInjuryCauseCode($input): string
    {
        if (str_contains($input, ',')) {
    ...
    public static function getAICommentary($prompt)
    {
        try {
                ...
                'messages' => [
                    [
                        ...
                        'content' => "Sen bir ISO 45001 Sertifikalı İş Güvenliği Uzmanısın. "
                            ...
                            . "1. Yaralanma nedenlerine göre iş kazaları ve ölüm verilerine odaklan\n"
                            . "2. ILO kaza sınıflandırmasını kullan\n"
            ...
            return "ISO 45001 uyumlu yaralanma nedeni analizi geçici olarak mevcut değil. Lütfen teknik ekibe bildiriniz.";
        }
    }
