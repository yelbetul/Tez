<?php
    ...
    class AnalysisHelperAccidentsByHours
    {
        public static function buildAIPrompt($request, $summary)
        {
            $year = self::formatRequestInput($request->input('year'), 'Tüm yıllar');
            $timeInterval = self::formatRequestInput($request->input('time_interval'), 'Tüm zaman aralıkları');
            $gender = self::formatRequestInput($request->input('gender'), 'Tüm cinsiyetler');

            $prompt = "Aşağıdaki zaman aralıklarına göre iş kazası verilerini ISO 45001 standartlarına uygun teknik raporda analiz et:\n\n";
            $prompt .= "VERİ PARAMETRELERİ (ISO 45003:2021 Formatında):\n";
            $prompt .= "- Analiz Periyodu: ".self::formatISODateRange($year)."\n";
            $prompt .= "- Zaman Aralığı: ".self::formatTimeInterval($timeInterval)."\n";
            ...
            $prompt .= "   - Zaman aralıklarına göre kaza sıklık oranı\n";
            $prompt .= "   - İş göremezlik sürelerine göre riskli saatler\n\n";

            $prompt .= "2. Kök Neden Analizi (ISO 45002:2016):\n";
            $prompt .= "   - Zaman aralıklarına özgü kaza nedenleri (Fishbone diyagramı)\n\n";

            $prompt .= "3. Demografik Analiz (ILO C161 Uyumlu):\n";
            $prompt .= "   - Saat aralıklarına göre cinsiyet dağılımı\n\n";

            $prompt .= "4. İyileştirme Planı (Hierarchy of Controls):\n";
            $prompt .= "   - Kritik saatler için ek önlemler (EN ISO 12100)\n";
            $prompt .= "   - Yorgunluk yönetimi protokolleri\n\n";

            $prompt .= "5. Risk Projeksiyonu:\n";
            $prompt .= "   - Gün içi kaza trend analizi\n\n";

            $prompt .= "RAPOR FORMAT KURALLARI:\n";
            $prompt .= "- Tamamen ISO 45001 terminolojisi kullan\n";
            $prompt .= "- Zaman aralıklarını belirt\n";
        ...
        private static function formatTimeInterval($input): string
        {
            if(str_contains($input, ',')){
                return "Çoklu Zaman Aralığı (".$input.")";
            }
            return $input;
        }
        ...
        public static function getAICommentary($prompt)
        {
            try{
                                                       ...
                                                       'messages' => [
                                                           [
                                                               ...
                                                               'content' => "Sen bir ISO 45001 Sertifikalı İş Güvenliği Uzmanısın. "
                                                                   ...
                                                                   ."1. Gün içi zaman aralıklarına göre iş kazaları verilerine odaklan\n"
                                                                   ."2. ILO çalışma saatleri sınıflandırmasını kullan\n"
                ...
                return "ISO 45001 uyumlu zaman aralığı risk analizi geçici olarak mevcut değil. Lütfen teknik ekibe bildiriniz.";
            }
        }
