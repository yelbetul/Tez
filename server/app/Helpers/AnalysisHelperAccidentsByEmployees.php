<?php
    ...
    class AnalysisHelperAccidentsByEmployees
    {
        public static function buildAIPrompt($request, $summary)
        {
            ...
            $prompt = "Aşağıdaki çalışan sayılarına göre iş kazası ve ölüm verilerini ISO 45001 standartlarına uygun teknik raporda analiz et:\n\n";
            $prompt .= "VERİ PARAMETRELERİ (ISO 45003:2021 Formatında):\n";
            $prompt .= "- Analiz Periyodu: ".self::formatISODateRange($year)."\n";
            $prompt .= "- Çalışan Sayısı Grubu: ".self::formatEmployeeGroup($employeeGroup)."\n";
            ...
            $prompt .= "   - Çalışan sayısı gruplarına göre kaza sıklık oranı\n";
            $prompt .= "   - İş göremezlik sürelerine göre riskli işletme büyüklükleri\n\n";

            $prompt .= "2. Kök Neden Analizi (ISO 45002:2016):\n";
            $prompt .= "   - Çalışan sayısına özgü kaza nedenleri (Fishbone diyagramı)\n\n";

            $prompt .= "3. Demografik Analiz (ILO C161 Uyumlu):\n";
            $prompt .= "   - İşletme büyüklüklerine göre cinsiyet dağılımı\n\n";

            $prompt .= "4. İyileştirme Planı (Hierarchy of Controls):\n";
            $prompt .= "   - İşletme büyüklüğüne göre ölçeklenebilir önlemler (EN ISO 12100)\n";
            $prompt .= "   - Çalışan sayısına göre iş sağlığı ve güvenliği organizasyonu\n\n";

            $prompt .= "5. Risk Projeksiyonu:\n";
            $prompt .= "   - İşletme büyüklüğüne göre 3 yıllık kaza trend analizi\n\n";

            $prompt .= "RAPOR FORMAT KURALLARI:\n";
            $prompt .= "- Tamamen ISO 45001 terminolojisi kullan\n";
            $prompt .= "- Çalışan sayısı gruplarını belirt\n";
        ...
        private static function generateQuantitativeDataSection(array $summary): string
        {
            return "KANTİTATİF VERİLER (ISO 39001 Formatında):\n".
                ...
                "- Ölümlü Vakalar: ".($summary['total_fatalities'] ?? 0)."\n".
                "- Cinsiyet Dağılımı:\n".
        ...
        private static function formatEmployeeGroup($input): string
        {
            if(str_contains($input, ',')){
                return "Çoklu Çalışan Grubu (".$input.")";
            }
            return $input;
        }
        ...
        public static function getAICommentary($prompt)
        {
            try{
                                                       ...
                                                       'messages' => [
                                                           [
                                                               ...
                                                               'content' => "Sen bir ISO 45001 Sertifikalı İş Güvenliği Uzmanısın. "
                                                                   ...
                                                                   ."1. Çalışan sayısı gruplarına göre iş kazaları verilerine odaklan\n"
                                                                   ."2. ILO işletme büyüklüğü sınıflandırmasını kullan\n"
                ...
                return "ISO 45001 uyumlu çalışan grubu risk analizi geçici olarak mevcut değil. Lütfen teknik ekibe bildiriniz.";
            }
        }
