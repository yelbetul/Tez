<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="material_codes-modal">
            <div class="close" @click="closeModal">
                <i class="fa-solid fa-xmark"></i>
            </div>
            <h2>Materyal Kodu {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                <div v-if="error" class="form-error">
                    <span><i class="fa-solid fa-xmark"></i> {{ errorMessage }}</span>
                </div>
                <div class="form-element">
                    <label for="material_code">Materyal Kodu</label>
                    <input type="text" id="material_code" v-model="formData.material_code"
                        placeholder="Materyal kodu giriniz...">
                </div>
                <div class="form-element">
