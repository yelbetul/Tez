<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="general-activities-modal">
            <div class="close" @click="closeModal">
                <i class="fa-solid fa-xmark"></i>
            </div>
            <h2>Genel Faaliyet Kodu {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                <div v-if="error" class="form-error">
                    <span><i class="fa-solid fa-xmark"></i> {{ errorMessage }}</span>
                </div>
                <div class="form-element">
                    <label for="general_activity_code">Genel Faaliyet Kodu</label>
                    <input type="text" id="general_activity_code" v-model="formData.general_activity_code"
                        placeholder="Genel faaliyet kodu giriniz...">
                </div>
                <div class="form-element">
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/general-activities/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            ...
                            Swal.fire({
                                title: 'Başarılı!',
                                text: 'Genel faaliyet kodu başarıyla eklendi.',
                                icon: 'success',
                            }).then(() => {
                                this.$emit('close');
                            });
                ...
                axios.put('https://iskazalarianaliz.com/api/general-activities/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
                            ...
                            Swal.fire({
                                title: 'Başarılı!',
                                text: 'Genel faaliyet kodu başarıyla güncellendi.',
                                icon: 'success',
                            }).then(() => {
                                this.$emit('close');
                            });
