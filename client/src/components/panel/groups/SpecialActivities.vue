<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="special-activities-modal">
            <div class="close" @click="closeModal">
                <i class="fa-solid fa-xmark"></i>
            </div>
            <h2>Özel Faaliyet Kodu {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                <div v-if="error" class="form-error">
                    <span><i class="fa-solid fa-xmark"></i> {{ errorMessage }}</span>
                </div>
                <div class="form-element">
                    <label for="special_activity_code">Özel Faaliyet Kodu</label>
                    <input type="text" id="special_activity_code" v-model="formData.special_activity_code"
                        placeholder="Özel faaliyet kodu giriniz...">
                </div>
                <div class="form-element">
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/special-activities-before-accidents/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            ...
                            Swal.fire({
                                title: 'Başarılı!',
                                text: 'Özel faaliyet kodu başarıyla eklendi.',
                                icon: 'success',
                            }).then(() => {
                                this.$emit('close');
                            });
                ...
                axios.put('https://iskazalarianaliz.com/api/special-activities-before-accidents/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
                            ...
                            Swal.fire({
                                title: 'Başarılı!',
                                text: 'Özel faaliyet kodu başarıyla güncellendi.',
                                icon: 'success',
                            }).then(() => {
                                this.$emit('close');
                            });
