<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="sector-codes-modal">
            ...
            <h2>Çalışma Saatlerine Göre İş Kazası Verisi {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                ...
                <div class="form-element">
                    <label for="time_interval">Çalışma Saatleri</label>
                    <select id="time_interval" v-model="formData.group_id">
                        <option value="0">Seçiniz...</option>
                        <option v-for="item in time_intervals" :key="item.id" :value="item.id">
                            {{ item.time_interval }}
                        </option>
                    </select>
                </div>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/work-accidents-by-hours/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                ...
                axios.put('https://iskazalarianaliz.com/api/work-accidents-by-hours/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
    ...
    created() {
        axios.get('https://iskazalarianaliz.com/api/time-intervals')
            .then(res => {
                this.time_intervals = res.data.data
            })
    },
}
</script>
