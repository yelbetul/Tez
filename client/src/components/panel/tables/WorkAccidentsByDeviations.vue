<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="sector-codes-modal">
            ...
            <h2>Sapmaya Göre İş Kazası Verisi {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                ...
                <div class="form-element">
                    ...
                    <select id="deviation" v-model="formData.group_id">
                        <option value="0">Seçiniz...</option>
                        <option v-for="item in deviations" :key="item.id" :value="item.id">
                            {{ item.deviation_code }}
                        </option>
                    </select>
                </div>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-deviations/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                ...
                axios.put('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-deviations/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
