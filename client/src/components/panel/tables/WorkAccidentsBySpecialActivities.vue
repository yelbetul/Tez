<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="sector-codes-modal">
            ...
            <h2>Özel Faaliyete Göre İş Kazası Verisi {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                ...
                <div class="form-element">
                    <label for="special_activity">Özel Faaliyet</label>
                    <select id="special_activity" v-model="formData.group_id">
                        <option value="0">Seçiniz...</option>
                        <option v-for="item in special_activities" :key="item.id" :value="item.id">
                            {{ item.special_activity_code }}
                        </option>
                    </select>
                </div>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-special-activities/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                ...
                axios.put('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-special-activities/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
