<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="sector-codes-modal">
            ...
            <h2>İllere Göre Ölümlü İş Kazası Verisi {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/fatal-work-accidents-by-province/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                ...
                axios.put('https://iskazalarianaliz.com/api/fatal-work-accidents-by-province/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
