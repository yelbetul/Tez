<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="sector-codes-modal">
            ...
            <h2>İllere Göre İş Göremezlik Verisi {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                ...
                <div class="form-element">
                    <label for="outpatient">Ayakta</label>
                    <input type="number" id="outpatient" v-model="formData.outpatient" min="0" />
                </div>

                <div class="form-element">
                    <label for="inpatient">Yatarak</label>
                    <input type="number" id="inpatient" v-model="formData.inpatient" min="0" />
                </div>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/disability-days-occ-dis-by-province/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                ...
                axios.put('https://iskazalarianaliz.com/api/disability-days-occ-dis-by-province/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
