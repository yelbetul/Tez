<template>
    <div v-if="visible" class="modal-overlay" @click.self="closeModal">
        <div class="sector-codes-modal">
            ...
            <h2>Sektöre Göre Ölümlü İş Kazası Verisi {{ state === 'new' ? 'Oluştur' : 'Güncelle' }}</h2>
            <form class="form" @submit.prevent="submitForm">
                ...
                <div class="form-element">
                    ...
                    <select id="sector_code" v-model="formData.group_id">
                        <option value="0">Seçiniz...</option>
                        <option v-for="item in sector_codes" :key="item.id" :value="item.id">
                            {{ item.sector_code }}
                        </option>
                    </select>
                </div>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.state == 'new') {
                axios.post('https://iskazalarianaliz.com/api/fatal-work-accidents-by-sector/store', this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                ...
                axios.put('https://iskazalarianaliz.com/api/fatal-work-accidents-by-sector/update/' + this.formData.id, this.formData)
                    .then(res => {
                        if (!res.data.success) {
                            this.error = true;
                            this.errorMessage = res.data.message;
