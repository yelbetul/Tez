  ...
  <div class="deviation-analysis-container">
    ...
    <div class="filters">
      ...
      <div class="filter-group">
        <label for="group">Sapma Grubu:</label>
    ...
    <div class="data-table" v-if="tableData.length > 0 && !loading">
      ...
      <table>
        ...
        <tbody>
        <tr v-for="item in tableData" :key="`${item.group_code}-${item.sub_group_code}-${item.deviation_code}-${item.year}`">
...
export default {
  ...
  setup() {
    ...
    // Sapma türleri hiyerarşisi için computed özellikler
    const availableGroups = computed(() => {
      const groups = new Map()
      deviationTypes.value.forEach(item => {
    ...
    const filteredDeviations = computed(() => {
      ...
      return deviationTypes.value
        .filter(item => item.sub_group_code === selectedSubGroup.value)
        .map(item => ({
          code: item.deviation_code,
          name: item.sub_group_name
        }))
    })

    const selectedDeviationCode = computed(() => {
      if (selectedDeviation.value === 'all') return ''
      return selectedDeviation.value
    })
    ...
    const fetchData = async () => {
      ...
      try {
        const params = {
          ...
          deviation_code: selectedDeviation.value !== 'all' ? selectedDeviation.value : undefined,
          gender: selectedGender.value !== 'all' ? selectedGender.value : undefined
        }

        const response = await axios.get('/api/accidents-and-fatalities-by-deviations-user', { params })
    ...
    // Sapma türlerini yükle
    const loadDeviationTypes = async () => {
      try {
        const response = await axios.get('/api/deviations-user')
        deviationTypes.value = response.data
      } catch (error) {
        console.error('Sapma türleri yüklenirken hata:', error)
      }
    }
    ...
    const updateMainChart = () => {
      ...
      // Sapma gruplarına göre verileri grupla
