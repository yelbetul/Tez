<template>
  <div class="month-analysis-container">
    <div class="page-header">
      <h1>Aylara Göre Ölümlü İş Kazaları Analizi</h1>
      <p class="subtitle">2019-2023 yılları arası aylık ölümlü iş kazası verileri</p>
    </div>
    ...
    <div class="filters">
      ...
      <div v-if="selectedMonth !== 'all'" class="filter-group">
    ...
    <!-- Ana Grafik -->
    <div class="chart-container" v-if="selectedMonth === 'all' && !loading">
      <h2>Ay Bazlı Ölüm Karşılaştırması</h2>
      <apexchart type="bar" height="500" :options="mainChartOptions" :series="mainSeries"></apexchart>
    </div>

    <!-- Detay Grafik (Tek ay seçildiğinde) -->
    <div class="detail-charts" v-if="selectedMonth !== 'all' && !loading">
      <div class="chart-container">
        <h2>{{ selectedMonth }} - Yıllara Göre Dağılım</h2>
        <apexchart type="line" height="350" :options="yearlyChartOptions" :series="yearlySeries"></apexchart>
      </div>

      <div class="chart-container">
        <h2>{{ selectedMonth }} - Cinsiyet Dağılımı</h2>
    ...
    <div class="data-table" v-if="tableData.length > 0 && !loading">
      ...
      <table>
        ...
        <tr>
          <th>Ay</th>
          <th>Yıl</th>
          <th>İş Kazası Kaynaklı Ölümler</th>
          <th>Meslek Hastalığı Kaynaklı Ölümler</th>
          <th>Toplam Ölüm</th>
          <th>Erkek</th>
          <th>Kadın</th>
        </tr>
        ...
        <tbody>
        <tr v-for="item in tableData" :key="`${item.month}-${item.year}`">
          <td>{{ item.month }}</td>
          <td>{{ item.year }}</td>
  ...
  setup() {
    ...
    const mainChartOptions = ref({
      ...
      xaxis: {
        type: 'category',
        title: { text: 'Ay' },
        categories: availableMonths.value
      },
      yaxis: {
      ...
      colors: ['#ef4444'], // Kırmızı renk ölüm verileri için daha uygun
      tooltip: {
        y: {
          formatter: function (val) {
            return val + ' ölüm'
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/fatal-work-accidents-by-month-user', { params })
    ...
    const updateMainChart = () => {
      ...
      tableData.value.forEach(item => {
        ...
        monthData[month].total += total
        monthData[month].accident += item.work_accident_fatalities || 0
        monthData[month].disease += item.occupational_disease_fatalities || 0
        monthData[month].male_count += item.male_count || 0
        monthData[month].female_count += item.female_count || 0
      })

      // Grafik verilerini güncelle
      mainSeries.value = [{
        name: getMetricTitle(),
        data: availableMonths.value.map(month => ({
          x: month,
          y: selectedMetric.value === 'total' ? monthData[month].total :
            selectedMetric.value === 'accident' ? monthData[month].accident :
              monthData[month].disease
        }))
      }]
