<template>
  <div class="injury-cause-analysis-container">
    <div class="page-header">
      <h1>Yaralanma Nedenlerine Göre İş Kazaları ve Ölümler Analizi</h1>
      <p class="subtitle">2019-2023 yılları arası yaralanma nedenine göre iş kazası ve ölüm verileri</p>
    </div>

    <div class="filters">
      <!-- Yaralanma Nedeni Grubu Seçimi -->
      <div class="filter-group">
        <label for="group">Yaralanma Nedeni Grubu:</label>
      ...
      <!-- Yaralanma Nedeni Seçimi -->
      <div class="filter-group" v-if="selectedSubGroup !== 'all'">
        <label for="injuryCause">Yaralanma Nedeni:</label>
        <select id="injuryCause" v-model="selectedInjuryCause" @change="fetchData">
          <option value="all">Tüm Yaralanma Nedenleri</option>
          <option v-for="cause in filteredInjuryCauses" :key="cause.code" :value="cause.code">{{ cause.code }} - {{ cause.name }}</option>
        </select>
      </div>
    ...
    <!-- Ana Grafik -->
    <div class="chart-container" v-if="selectedInjuryCause === 'all' && !loading">
      <h2>Yaralanma Nedeni Gruplarına Göre Dağılım</h2>
      <apexchart type="bar" height="500" :options="mainChartOptions" :series="mainSeries"></apexchart>
    </div>

    <!-- Detay Grafik (Tek yaralanma nedeni seçildiğinde) -->
    <div class="detail-charts" v-if="selectedInjuryCause !== 'all' && !loading">
      <div class="chart-container">
        <h2>{{ selectedInjuryCauseCode }} - Yıllara Göre Dağılım</h2>
        <apexchart type="line" height="350" :options="yearlyChartOptions" :series="yearlySeries"></apexchart>
      </div>

      <div class="chart-container">
        <h2>{{ selectedInjuryCauseCode }} - Cinsiyet Dağılımı</h2>
    ...
    <div class="data-table" v-if="tableData.length > 0 && !loading">
      ...
      <table>
        <thead>
        <tr>
          <th>Yaralanma Nedeni Grubu</th>
          <th>Alt Grup</th>
          <th>Yaralanma Nedeni Kodu</th>
          <th>Yıl</th>
        ...
        <tbody>
        <tr v-for="item in tableData" :key="`${item.group_code}-${item.sub_group_code}-${item.injury_cause_code}-${item.year}`">
...
export default {
  ...
  setup() {
    ...
    const injuryCauses = ref([])

    // Seçimler
    const selectedYear = ref('all')
    const selectedGroup = ref('all')
    const selectedSubGroup = ref('all')
    const selectedInjuryCause = ref('all')
    ...
    // Yaralanma nedenleri hiyerarşisi için computed özellikler
    const availableGroups = computed(() => {
      const groups = new Map()
      injuryCauses.value.forEach(item => {
    ...
    const filteredInjuryCauses = computed(() => {
      if (selectedSubGroup.value === 'all') return []
      return injuryCauses.value
        .filter(item => item.sub_group_code === selectedSubGroup.value)
        .map(item => ({
          code: item.injury_cause_code,
          name: item.sub_group_name
        }))
    })

    const selectedInjuryCauseCode = computed(() => {
      if (selectedInjuryCause.value === 'all') return ''
      return selectedInjuryCause.value
    })
    ...
    const mainChartOptions = ref({
      ...
      xaxis: {
        type: 'category',
        title: { text: 'Yaralanma Nedeni Grupları' },
        categories: []
      },
      yaxis: {
    ...
    const fetchData = async () => {
      ...
      try {
        const params = {
          ...
          injury_cause_code: selectedInjuryCause.value !== 'all' ? selectedInjuryCause.value : undefined,
          gender: selectedGender.value !== 'all' ? selectedGender.value : undefined
        }

        const response = await axios.get('/api/accidents-and-fatalities-by-injury-causes-user', { params })
    ...
    // Yaralanma nedenlerini yükle
    const loadInjuryCauses = async () => {
      try {
        const response = await axios.get('/api/injury-causes-user')
        injuryCauses.value = response.data
      } catch (error) {
        console.error('Yaralanma nedenleri yüklenirken hata:', error)
      }
    }

    // Grafik güncelleme fonksiyonları
    const updateCharts = () => {
      if (selectedInjuryCause.value === 'all') {
        updateMainChart()
      } else {
        updateDetailCharts()
      }
    ...
    const updateMainChart = () => {
      ...
      // Yaralanma nedeni gruplarına göre verileri grupla
    ...
    return {
      ...
      availableGroups,
      filteredSubGroups,
      filteredInjuryCauses,
      selectedYear,
      selectedGroup,
      selectedSubGroup,
      selectedInjuryCause,
      selectedInjuryCauseCode,
      selectedGender,
      selectedMetric,
      mainChartOptions,
      mainSeries,
...
<style scoped>
.injury-cause-analysis-container {
    max-width: 90%;
    margin: 0 auto;
    padding: 20px;
}
