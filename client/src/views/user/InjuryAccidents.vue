<template>
  <div class="injury-analysis-container">
    <div class="page-header">
      <h1>Yaralanma Türlerine Göre İş Kazaları ve Ölümler Analizi</h1>
      <p class="subtitle">2019-2023 yılları arası yaralanma türüne göre iş kazası ve ölüm verileri</p>
    </div>

    <div class="filters">
      <!-- Yaralanma Grubu Seçimi -->
      <div class="filter-group">
        <label for="group">Yaralanma Grubu:</label>
      ...
      <!-- Yaralanma Türü Seçimi -->
      <div class="filter-group" v-if="selectedSubGroup !== 'all'">
        <label for="injury">Yaralanma Türü:</label>
        <select id="injury" v-model="selectedInjury" @change="fetchData">
          <option value="all">Tüm Yaralanma Türleri</option>
          <option v-for="injury in filteredInjuryTypes" :key="injury.code" :value="injury.code">{{ injury.code }} - {{ injury.name }}</option>
        </select>
      </div>
    ...
    <!-- Ana Grafik -->
    <div class="chart-container" v-if="selectedInjury === 'all' && !loading">
      <h2>Yaralanma Gruplarına Göre Dağılım</h2>
      <apexchart type="bar" height="500" :options="mainChartOptions" :series="mainSeries"></apexchart>
    </div>

    <!-- Detay Grafik (Tek yaralanma türü seçildiğinde) -->
    <div class="detail-charts" v-if="selectedInjury !== 'all' && !loading">
      <div class="chart-container">
        <h2>{{ selectedInjuryCode }} - Yıllara Göre Dağılım</h2>
        <apexchart type="line" height="350" :options="yearlyChartOptions" :series="yearlySeries"></apexchart>
      </div>

      <div class="chart-container">
        <h2>{{ selectedInjuryCode }} - Cinsiyet Dağılımı</h2>
    ...
    <div class="data-table" v-if="tableData.length > 0 && !loading">
      ...
      <table>
        <thead>
        <tr>
          <th>Yaralanma Grubu</th>
          <th>Alt Grup</th>
          <th>Yaralanma Kodu</th>
          <th>Yıl</th>
        ...
        <tbody>
        <tr v-for="item in tableData" :key="`${item.group_code}-${item.sub_group_code}-${item.injury_code}-${item.year}`">
...
export default {
  ...
  setup() {
    ...
    // Yaralanma türleri hiyerarşisi için computed özellikler
    const availableGroups = computed(() => {
      const groups = new Map()
      injuryTypes.value.forEach(item => {
    ...
    const filteredInjuryTypes = computed(() => {
      if (selectedSubGroup.value === 'all') return []
      return injuryTypes.value
        .filter(item => item.sub_group_code === selectedSubGroup.value)
        .map(item => ({
          code: item.injury_code,
          name: item.sub_group_name
        }))
    })

    const selectedInjuryCode = computed(() => {
      if (selectedInjury.value === 'all') return ''
      return selectedInjury.value
    })
    ...
    const fetchData = async () => {
      ...
      try {
        const params = {
          ...
          injury_code: selectedInjury.value !== 'all' ? selectedInjury.value : undefined,
          gender: selectedGender.value !== 'all' ? selectedGender.value : undefined
        }

        const response = await axios.get('/api/accidents-and-fatalities-by-injury-user', { params })
    ...
    // Yaralanma türlerini yükle
    const loadInjuryTypes = async () => {
      try {
        const response = await axios.get('/api/injury-types-user')
        injuryTypes.value = response.data
      } catch (error) {
        console.error('Yaralanma türleri yüklenirken hata:', error)
      }
    }

    // Grafik güncelleme fonksiyonları
    const updateCharts = () => {
      if (selectedInjury.value === 'all') {
        updateMainChart()
      } else {
        updateDetailCharts()
      }
    ...
    const updateMainChart = () => {
      ...
      // Yaralanma gruplarına göre verileri grupla
