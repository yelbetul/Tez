<template>
  <div class="workstation-analysis-container">
    ...
    <div class="page-header">
      <h1>Çalışan Gruplarına Göre İş Kazaları Analizi</h1>
      <p class="subtitle">2019-2023 yılları arası çalışan gruplarına göre iş kazası verileri</p>
    </div>
  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/accidents-and-fatalities-by-employee-user', { params })
    ...
    const updateMainChart = () => {
      ...
      tableData.value.forEach(item => {
        ...
        groupData[item.code].total += calculateTotal(item)
        groupData[item.code].unfit += calculateUnfit(item)
        groupData[item.code].fatalities += item.fatalities || 0
      ...
      availableEmployeeGroups.value.forEach(group => {
        if (groupData[group.code]) {
          ...
          switch (selectedMetric.value) {
            case 'total': value = groupData[group.code].total; break
            case 'unfit': value = groupData[group.code].unfit; break
    ...
    const updateYearlyTrendChart = () => {
      ...
      tableData.value.forEach(item => {
        let value = 0
        switch (selectedMetric.value) {
          case 'total': value = calculateTotal(item); break
          case 'unfit': value = calculateUnfit(item); break
          case 'fatalities': value = item.fatalities || 0; break
        }
