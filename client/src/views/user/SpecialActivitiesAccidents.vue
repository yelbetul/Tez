  ...
  <div class="injury-analysis-container">
    ...
    <div class="data-table" v-if="tableData.length > 0 && !loading">
      ...
      <table>
        ...
        <tbody>
        <tr v-for="item in tableData" :key="`${item.group_code}-${item.sub_group_code}-${item.special_activity_code}-${item.year}`">
  ...
  setup() {
    ...
    const filteredActivities = computed(() => {
      ...
      return activityTypes.value
        ...
        .map(item => ({
          code: item.special_activity_code,
          name: item.sub_group_name
        }))
    })
    ...
    const fetchData = async () => {
      ...
      try {
        const params = {
          ...
          special_activity_code: selectedActivity.value !== 'all' ? selectedActivity.value : undefined,
    ...
    const loadActivityTypes = async () => {
      try {
        const response = await axios.get('/api/special-activities-user')
