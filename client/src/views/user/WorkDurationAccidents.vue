<template>
  <div class="workstation-analysis-container">
    ...
    <div class="page-header">
      <h1>Çalışma Süresine Göre İş Kazaları ve Meslek Hastalıkları Analizi</h1>
      <p class="subtitle">Çalışanların çalışma sürelerine göre iş kazaları ve meslek hastalıkları verileri</p>
    </div>
  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/occ-disease-fatalities-by-employer-durations-user', { params })
    ...
    const updateMainChart = () => {
      ...
      tableData.value.forEach(item => {
        ...
        durationData[item.code].cases += item.occ_disease_cases || 0
        durationData[item.code].fatalities += item.occ_disease_fatalities || 0
      ...
      availableEmploymentDurations.value.forEach(duration => {
        if (durationData[duration.code]) {
          ...
          switch (selectedMetric.value) {
            case 'cases': value = durationData[duration.code].cases; break
            case 'fatalities': value = durationData[duration.code].fatalities; break
          }
