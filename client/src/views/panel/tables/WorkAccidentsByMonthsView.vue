<template>
    ...
    <WorkAccidentsByMonths v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByMonths from '@/components/panel/tables/WorkAccidentsByMonths.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByMonths.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByMonths,
        ImportData
    },
    setup() {
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/work-accidents-by-month/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Aylara Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Yaş', key: 'month', width: 15 },
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Aylara_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/work-accidents-by-month')
