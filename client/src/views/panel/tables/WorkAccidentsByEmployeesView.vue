<template>
    ...
    <WorkAccidentsByEmployees v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByEmployees from '@/components/panel/tables/WorkAccidentsByEmployees.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByEmployees.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByEmployees,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Çalışan Sayısına Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-employee/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            worksheet.columns = [
                ...
                { header: 'Çalışan Sayısı', key: 'employee_code', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    employee_code: item.employee_group.employee_count,  // Yara Kodu
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-employee')
