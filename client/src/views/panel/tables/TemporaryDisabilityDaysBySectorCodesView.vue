<template>
    ...
    <TemporaryDisabilityDaysBySectorCodes v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
</template>
...
import TemporaryDisabilityDaysBySectorCodes from '@/components/panel/tables/TemporaryDisabilityDaysBySectorCodes.vue';
export default {
    components: {
        PageNavbar,
    ...
    data() {
        return {
            navbarData: {
                title: 'Sektörlere Göre Geçici İş Göremezlik Süreleri',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/temporary-disability-day-by-sector/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('Geçici İş Göremezlik Sektöre Göre');
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Gecici_Is_Göremezlik_Sektor_Gore.xlsx';
