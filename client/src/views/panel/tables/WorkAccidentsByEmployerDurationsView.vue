<template>
    ...
    <WorkAccidentsByEmployerDurations v-if="modal_visible" :visible="modal_visible" :data="selected_code"
...
import WorkAccidentsByEmployerDurations from '@/components/panel/tables/WorkAccidentsByEmployerDurations.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByEmployerDurations.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByEmployerDurations,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Çalışma Süresine Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-employer-durations/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Çalışan Sayısına Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Çalışma Süresi', key: 'employer_durations', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    employer_durations: item.employee_employment_duration.employment_duration,  // Yara Kodu
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Calisma_Suresine_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-employer-durations')
