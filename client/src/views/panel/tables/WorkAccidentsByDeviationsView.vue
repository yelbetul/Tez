<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.deviation.deviation_code }}</td>
    ...
    <WorkAccidentsByDeviations v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByDeviations from '@/components/panel/tables/WorkAccidentsByDeviations.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByDeviations.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByDeviations,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Sapmalara Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-deviations/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Sapmalara Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Sapma Kodu', key: 'deviation_code', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    deviation_code: item.deviation.deviation_code,  // Yara Kodu
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Sapmalara_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-deviations')
