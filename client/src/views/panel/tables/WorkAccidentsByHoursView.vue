<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.time_interval?.time_interval }}</td>
    ...
    <WorkAccidentsByHour v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByHour from '@/components/panel/tables/WorkAccidentsByHour.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByHour.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByHour,
    ...
    data() {
        return {
            navbarData: {
                title: 'Çalışma Saatlerine Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/work-accidents-by-hours/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Çalışma Saatlerine Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Çalışma Saatleri', key: 'time_interval', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    time_interval: item.time_intervals.time_interval,  // İl
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Calisma_Saatlerine_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/work-accidents-by-hours')
