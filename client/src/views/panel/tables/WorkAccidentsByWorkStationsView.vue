<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.workstation_type?.workstation_code }}</td>
    ...
    <WorkAccidentsByWorkstations v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByWorkstations from '@/components/panel/tables/WorkAccidentsByWorkstations.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByWorkstations.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByWorkstations,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Çalışılan Ortamlara Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/work-accidents-by-work-stations/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Çalışılan Ortama Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Çalışılan Ortam', key: 'work_stations', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    work_stations: item.workstation_type.workstation_code,  // İl
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Calisilan_Ortama_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/work-accidents-by-work-stations')
