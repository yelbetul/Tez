<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.injury_type.injury_code }}</td>
    ...
    <WorkAccidentsByInjuryTypes v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByInjuryTypes from '@/components/panel/tables/WorkAccidentsByInjuryTypes.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByInjuryTypes.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByInjuryTypes,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Yaranın Türüne Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-injury/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            worksheet.columns = [
                ...
                { header: 'Yaranın Türü', key: 'injury_code', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    injury_code: item.injury_type.injury_code,  // Yara Kodu
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Yaranin_Turune_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-injury')
