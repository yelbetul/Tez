<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.work_environment.environment_code }}</td>
    ...
    <WorkAccidentsByEnvironments v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByEnvironments from '@/components/panel/tables/WorkAccidentsByEnvironments.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByEnvironments.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByEnvironments,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Çalışılan Çevreye Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-environments/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Çalışılan Çevreye Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Çalışılan Çevre', key: 'environment_code', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    environment_code: item.work_environment.environment_code,  // Yara Kodu
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Calisilan_Cevreye_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-environments')
