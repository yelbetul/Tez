<template>
    ...
    <FatalWorkAccidentsByMonths v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import FatalWorkAccidentsByMonths from '@/components/panel/tables/FatalWorkAccidentsByMonths.vue';
import ImportData from '@/components/panel/tables/import/FatalWorkAccidentsByMonths.vue';
export default {
    components: {
        PageNavbar,
        FatalWorkAccidentsByMonths,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Aylara Göre Ölümlü İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/fatal-work-accidents-by-month/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('Ölümlü İş Kazaları Aylara Göre');
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Olumlu_Is_Kazalari_Aylara_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/fatal-work-accidents-by-month')
