<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.diagnosis_group.diagnosis_code }}</td>
    ...
    <OccupationalDiseaseByDiagnosis v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import OccupationalDiseaseByDiagnosis from '@/components/panel/tables/OccupationalDiseaseByDiagnosis.vue';
import ImportData from '@/components/panel/tables/import/OccupationalDiseaseByDiagnosis.vue';
export default {
    components: {
        PageNavbar,
        OccupationalDiseaseByDiagnosis,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Tanılara Göre Meslek Hastalığına Yakalananlar',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/occupational-disease-by-diagnosis/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('Tanılara Göre Meslek Hastalığına Yakalanan');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Yaş', key: 'diagnosis_code', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    diagnosis_code: item.diagnosis_group.diagnosis_code,
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Tanilara_Gore_Meslek_Hastaligina_Yakalananlar.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/occupational-disease-by-diagnosis')
