<template>
        ...
        <div class="table-container" v-if="!isLoading && !error">
            <table>
                <thead>
                    ...
                    <td>Ölüm <br> <span>Kaza Sonucu</span></td>
                    <td style="width: 10%;"></td>
                </thead>
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.injury_location.injury_location_code }}</td>
    ...
    <WorkAccidentsByInjuryLocations v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsByInjuryLocations from '@/components/panel/tables/WorkAccidentsByInjuryLocations.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentByInjuryLocations.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsByInjuryLocations,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Yaranın Vücuttaki Yerine Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-injury-locations/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Kazaları Yara Türüne Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'Yaranın Türü', key: 'injury_location_code', width: 15 },
                ...
                { header: 'Meslek Hastalığına Yakalanan', key: 'fatalities', width: 25 },
            ];
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    injury_location_code: item.injury_location.injury_location_code,  // Yara Kodu
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Kazalari_Yaranin_Yerine_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-injury-locations')
