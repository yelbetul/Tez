<template>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredData" :key="item.id">
                        <td>{{ item.year }}</td>
                        <td>{{ item.special_activity_before_accident.special_activity_code }}</td>
    ...
    <WorkAccidentsBySpecialActivities v-if="modal_visible" :visible="modal_visible" :data="selected_code" :state="state"
        @close="closeModal" />
...
import WorkAccidentsBySpecialActivities from '@/components/panel/tables/WorkAccidentsBySpecialActivities.vue';
import ImportData from '@/components/panel/tables/import/WorkAccidentBySpecialActivities.vue';
export default {
    components: {
        PageNavbar,
        WorkAccidentsBySpecialActivities,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'Özel Faaliyete Göre İş Kazaları',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-special-activities/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            worksheet.columns = [
                ...
                { header: 'Özel Faaliyet', key: 'special_activity_code', width: 15 },
            ...
            this.data.forEach((item) => {
                worksheet.addRow({
                    year: item.year,
                    special_activity_code: item.special_activity_before_accident.special_activity_code,  // Yara Kodu
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/accidents-and-fatalities-by-special-activities')
