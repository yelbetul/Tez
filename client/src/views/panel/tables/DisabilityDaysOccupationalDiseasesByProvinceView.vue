<template>
    ...
    <DisabilityDaysOccupationalDiseasesByProvince v-if="modal_visible" :visible="modal_visible" :data="selected_code"
...
import DisabilityDaysOccupationalDiseasesByProvince from '@/components/panel/tables/DisabilityDaysOccupationalDiseasesByProvince.vue';
import ImportData from '@/components/panel/tables/import/DisabilityDaysOccupationalDiseasesByProvince.vue';
export default {
    components: {
        PageNavbar,
        DisabilityDaysOccupationalDiseasesByProvince,
        ImportData
    },
    setup() {
    ...
    data() {
        return {
            navbarData: {
                title: 'İllere Göre İş Göremezlik Süresi',
    ...
    methods: {
        ...
        deleteData(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/disability-days-occ-dis-by-province/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('İş Göremezlik Süreleri İllere Göre');

            worksheet.columns = [
                { header: 'Yıl', key: 'year', width: 10 },
                { header: 'İl', key: 'province', width: 15 },
                { header: 'Cinsiyet', key: 'gender', width: 10 },
                { header: 'Ayakta', key: 'outpatient', width: 15 },
                { header: 'Yatarak', key: 'inpatient', width: 20 },
            ];
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                a.href = url;
                a.download = 'Is_Goremezlik_Illere_Gore.xlsx';
        ...
        async initializeAuth() {
            ...
            try {
                ...
                const response = await axios.get('https://iskazalarianaliz.com/api/disability-days-occ-dis-by-province')
