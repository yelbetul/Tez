<template>
    <div class="page-container">
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    <tr v-for="item in special_activities" :key="item.id">
                        <td>{{ item.special_activity_code }}</td>
                        <td>{{ item.group_code }}</td>
                        <td>{{ item.group_name }}</td>
                        <td>{{ item.sub_group_code }}</td>
                        <td>{{ item.sub_group_name }}</td>
                        <td>
                            <i @click="updateSpecialActivity(item)" class="fa-solid fa-pen-to-square"></i>
                            <i @click="deleteSpecialActivity(item)" class="fa-solid fa-trash-can"></i>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <SpecialActivity v-if="modal_visible" :visible="modal_visible" :data="selected_activity" :state="state"
        @close="closeModal" />
</template>

<script>
import SpecialActivity from '@/components/panel/groups/SpecialActivities.vue';
...
export default {
    ...
    methods: {
        ...
        deleteSpecialActivity(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/special-activities-before-accidents/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
                                    title: 'Silindi!',
                                    text: 'Özel faaliyet kodu başarıyla silindi.',
                                    icon: 'success',
                                });
                            }
                        })
        ...
        async initializeAuth() {
            ...
            axios.get('https://iskazalarianaliz.com/api/special-activities-before-accidents')
                .then(res => {
                    this.special_activities = res.data.data
                })
        },
    },
    created() {
