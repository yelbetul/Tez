        ...
        <div class="table-container">
            <div class="table">
                <table>
                    ...
                    <tbody>
                        <tr v-for="item in employee_groups" :key="item.id">
                            <td>{{ item.code }}</td>
                            <td>{{ item.employee_count }}</td>
                            <td>
                                <i @click="updateModal(item)" class="fa-solid fa-pen-to-square"></i>
                                <i @click="deleteGroupCode(item)" class="fa-solid fa-trash-can"></i>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <EmployeeGroups v-if="modal_visible" :visible="modal_visible" :state="state" :data="update_group"
...
<script>
import EmployeeGroups from '@/components/panel/groups/EmployeeGroups.vue';
...
export default {
    ...
    methods: {
        updateModal(item) {
            this.update_group = item
            this.state = 'update'
        },
        updateState() {
            this.state = 'new'
            this.update_group = null
        },
        closeModal() {
            this.state = 'new'
            this.initializeAuth()
        },
        deleteGroupCode(item) {
            Swal.fire({
                title: 'Emin misiniz?',
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/employee-groups/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                            }
        ...
        async initializeAuth() {
            ...
            axios.get('https://iskazalarianaliz.com/api/employee-groups')
                .then(res => {
                    this.employee_groups = res.data.data
                })
        },
    },
    created() {
