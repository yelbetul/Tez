<template>
    <div class="page-container">
        ...
        <div class="navbar-buttons">
            <span @click="newDiagnosisGroups"><i class="fa-solid fa-plus"></i> Yeni Tanı</span>
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    <tr v-for="item in diagnosis" :key="item.id">
                        ...
                        <td>
                            <i @click="updateDiagnosisGroups(item)" class="fa-solid fa-pen-to-square"></i>
    ...
    <Diagnosis v-if="modal_visible" :visible="modal_visible" :data="selected_diagnosy" :state="state"
        @close="closeModal" />
</template>

<script>
import Diagnosis from '@/components/panel/groups/DiagnosisGroups.vue';
...
export default {
    ...
    methods: {
        ...
        updateDiagnosisGroups(item) {
            this.state = 'update'
            this.modal_visible = true
            this.selected_diagnosy = item
        },
        closeModal() {
            this.state = null
            this.modal_visible = false
            this.selected_diagnosy = null
            this.initializeAuth()
        },
        deleteInjuryCause(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('https://iskazalarianaliz.com/api/diagnosis-groups/delete/' + item.id)
                        .then(res => {
                            if (res.data.success) {
                                this.initializeAuth()
                                Swal.fire({
                                    title: 'Silindi!',
                                    text: 'Tanı başarıyla silindi.',
                                    icon: 'success',
                                });
                            }
                        })
        ...
        downloadExcel() {
            ...
            const worksheet = workbook.addWorksheet('Tanı');

            worksheet.columns = [
                { header: 'Tanı', key: 'diagnosis_code', width: 20 },
        ...
        async initializeAuth() {
            ...
            axios.get('https://iskazalarianaliz.com/api/diagnosis-groups')
                .then(res => {
